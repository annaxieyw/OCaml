...
and command =
  ...
  | FunEnd of value * value * commands
  | Return
  | Call
...
and funEnd (): command parser =
  let* _ = reserved "Fun" in
  let* cst = value << ws in
  let* dst = value << ws in
  let* coms = commands () in
  let* _ = reserved "EndFun" in
  return (FunEnd (cst, dst, coms))
...
let rec interp coms stack env =
  ...
  | (IfThenElse (coms1, coms2, coms3)) :: coms, _ ->
    (match interp coms1 stack env with
     ...
     | Exit stack -> Exit stack
     | error -> interp coms (E :: stack) env)
  | (FunEnd(v, arg, local)) :: coms, _ ->
    (match get_name v with
     | Some v -> interp coms (U :: stack) ((v, C(arg, local, env)) :: env)
     | error -> interp coms (E :: stack) env)
  | Return :: coms, x :: stack ->

    (match get x env with
     | Some x -> interp coms (x :: stack) env
     | Some E -> interp coms stack env)
  | Call :: coms, arg :: (N func) :: stack' ->
    (match get arg env, get_closure (N func) env with
     | Some arg, Some ((N argname),fcoms,fenv) -> 
        (match interp fcoms stack' ((func, C((N argname), fcoms, fenv)) :: (argname, arg) :: fenv) with
         | Ok (sv:: _) -> interp coms (sv :: stack') env
         | Exit stack -> Exit stack
         | error -> interp coms (E :: stack) env)
     | Some E, Some (argname,fcoms,fenv) -> interp coms (E :: stack) env
     | error -> interp coms (E :: stack) env)
  | (TryWith (coms1, coms2)) :: coms, _ -> (
    try (
      (match interp coms1 stack env with
      | Ok (sv) -> (
        if (List.mem E sv) then
          raise TryWithError
        else
          interp coms ((List.hd sv) :: stack) env
      )
      | Exit stack -> Exit stack
      | error -> interp coms (E :: stack) env)
    )
